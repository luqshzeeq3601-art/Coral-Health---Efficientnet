...
# ==========================================
# ResNet50 Robust Single-Seed Config
# ==========================================
IMG_SIZE = 224
...
GPU_MAX_TEMP_C = 80
GPU_RESUME_TEMP_C = 70
GPU_COOLDOWN_SECONDS = 30
...
class GPUTemperatureCallback(tf.keras.callbacks.Callback):
    ...
    def _write_status(self, event, epoch, note=""):
    ...
    def _cooldown_if_needed(self, epoch, phase):
        if not self.enabled:
            return
        status = self._write_status(f"{phase}_check", epoch)
        if status is None:
            return
        ...
        while status["temperature_c"] is not None and status["temperature_c"] >= self.max_temp_c:
            ...
            self._write_status("cooldown_wait", epoch, note)
            time.sleep(self.cooldown_seconds)
            status = self._write_status("cooldown_recheck", epoch)
            if status is None:
                return

        if status["temperature_c"] is not None and status["temperature_c"] <= self.resume_temp_c:
            self._write_status("cooldown_clear", epoch, f"temperature <= {self.resume_temp_c}C")
...
def build_model(weights="imagenet"):
    base_model = ResNet50(include_top=False, weights=weights, input_shape=(IMG_SIZE, IMG_SIZE, 3))
...
class AugmentedMixupGenerator(tf.keras.utils.Sequence):
    ...
    def __getitem__(self, index):
        ...
        if batch_size < 2:
            return preprocess_input(X_batch.copy()), y_batch
        ...
        X_mixed = preprocess_input(X_mixed.astype("float32").copy())
...
def save_training_history(history):
    ...
    with open(os.path.join(OUTPUT_DIR, "training_history_resnet50.json"), "w", encoding="utf-8") as f:
    ...
    ax1.set_title("ResNet50 Training and Validation Accuracy", fontsize=14, fontweight="bold")
    ...
    ax2.set_title("ResNet50 Training and Validation Loss", fontsize=14, fontweight="bold")
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(OUTPUT_DIR, "training_history_resnet50.png"), dpi=150, bbox_inches="tight")
    plt.close()


def evaluate_swa_model(model, X_test, y_test):
    X_test_preprocessed = preprocess_input(X_test.copy())
    preds = model.predict(X_test_preprocessed, batch_size=BATCH_SIZE, verbose=1)
    ...
    with open(os.path.join(OUTPUT_DIR, "classification_report_resnet50.txt"), "w", encoding="utf-8") as f:
        f.write("ResNet50 Robust Single Model - Seed 42 SWA\n")
    ...
    cm_payload = {
        "model": "ResNet50 Robust Single Model",
        "seed": SEED,
    ...
    with open(os.path.join(OUTPUT_DIR, "confusion_matrix_resnet50.json"), "w", encoding="utf-8") as f:
...
def train_model():
    ...
    print("=" * 60)
    print("RESNET50 ROBUST TRAINING - Single Seed 42")
    print("=" * 60)
    ...
    X_val_preprocessed = preprocess_input(X_val.copy())
    ...
    print("\nTraining Config:")
    print("  Model: ResNet50 Robust Single Model")
    ...
    print(f"  GPU Temp Control: max {GPU_MAX_TEMP_C}C, resume {GPU_RESUME_TEMP_C}C")
    ...
    with open(os.path.join(OUTPUT_DIR, "run_summary_resnet50.json"), "w", encoding="utf-8") as f:
        json.dump(run_summary, f, indent=4)

    loss_fn = tf.keras.losses.CategoricalCrossentropy(label_smoothing=LABEL_SMOOTHING)
    model.compile(optimizer=Adam(learning_rate=INITIAL_LR), loss=loss_fn, metrics=["accuracy"])

    model_path = os.path.join(MODEL_DIR, "resnet50_robust_seed42.weights.h5")
    swa_path = os.path.join(MODEL_DIR, "resnet50_robust_seed42_swa.weights.h5")
    ...
    print("\n" + "=" * 60)
    print("ResNet50 Robust Training Complete")
